...
def get_lowlevel_for_chunk(ids, rec_chunk, low_level):
...
def get_lowlevel_for_all(count_mapping):
    ...
    for ids, counts in count_mapping.items():
        ...
        if counts != None: 
            ...
            low_level = {}
            rec_list = get_rec_list(ids, counts)

            for rec_chunk in list(chunks(rec_list, 10)):    
            	get_lowlevel_for_chunk(ids, rec_chunk, low_level)
